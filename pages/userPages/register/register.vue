<template>
	<view class="content u-flex-col u-col-center">
		<switch-lange-header :title="$t('login.txt4')">
			<view slot="left" @click="jumpToLogin">
		...
		<view class="login-form width-688">
			...
			<view class="u-flex-col" style="z-index: 9;position: relative;">
				...
				<view class="px-font-14 input-item px-m-b-50">
					<u-input type="text" height="88" v-model="username" :placeholder="$t('reg.txt3')" />
				</view>
				<!-- <view class="input-label">{{$t('reg.txt4')}}</view> -->
				<view class="px-font-14 input-item px-m-b-50">
					<u-input type="text" height="88" v-model="userPhone" :placeholder="$t('reg.txt4')" />
				</view>
				<!-- <view class="input-label">{{$t('reg.txt5')}}</view> -->
				<view class="px-font-14 input-item px-m-b-50">
					<u-input type="password" height="88" v-model="password" :placeholder="$t('reg.txt5')" />
				</view>
				<!-- <view class="input-label">{{$t('reg.txt6')}}</view> -->
				<view class="px-font-14 input-item px-m-b-50">
					<u-input type="password" height="88" v-model="confirmPsd" :placeholder="$t('reg.txt6')" />
				</view>
			<!-- 	<view class="input-label">{{$t('reg.txt7')}}</view> -->
				<view class="px-font-14 input-item px-m-b-50">
					<u-input type="password" height="88" v-model="payPsd" :placeholder="$t('reg.txt7')" />
				</view>
				<!-- <view class="input-label">{{$t('reg.txt8')}}</view> -->
				<view class="px-font-14 input-item px-m-b-30">
					<u-input type="text" height="88" v-model="inviteCode" :placeholder="$t('reg.txt8')" />
				</view>
				...
				<view>
					<checkbox-group @change="checkboxChange">
						<label class="u-flex" style="color: #092C4C;">
							<checkbox style="transform:scale(0.6)" value="1" color="#3F8AD1" :checked="agree" />
							{{$t('reg.txt9')}} <text class="px-m-l-10" @click.stop="jumpTerm">{{$t('reg.txt10')}}</text>
						</label>
					</checkbox-group>
				</view>
	...
	export default {
		...
		methods: {
			...
			judge() {
				...
				if (this.username == "" || this.userPhone == "" || this.password == "" || this.confirmPsd == "" || this
					.payPsd == "" || this.gender == "" || this.inviteCode == "") {
