<template>
	<view class="content">
		...
		<view class="list">
			<view v-if="orderList.length>0">
				<view class="list-item" v-for="(item,index) in orderList" :key="index">
					...
					<u-button v-if="item.status==1" hover-class="none" :disabled="showLoading" :hair-line="false" :loading="showLoading" class="btn" @click="subOrder(item.id)">{{$t('history.txt3')}}</u-button>
				</view>
				<u-loadmore :status="loadStatus" :load-text="loadText" />
			</view>
			<view class="px-m-t-100" v-else>
				<u-empty :text="$t('common.empty')" src="/static/images/common/empty.png" icon-size="518"></u-empty>
			</view>
		</view>
		<!-- 自定义loading -->
		...
		<!-- modal -->
		<h-modal ref="HModal" :tipsContent="$t('history.txt6')" :logo="vuex_config.app_logo" @confirm="submissionHandle"></h-modal>
		<!-- 自定义tabbar -->
		<h-tabbar :activeIndex="2"></h-tabbar>
	</view>
</template>
