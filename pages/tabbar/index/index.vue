<template>
	<l-base
		:logo="homeData.config && homeData.config.app_logo"
		:activeIndex="0"
		:scrollDis="scrollDis"
	>
		<view class="content u-flex-col u-col-center">
			...
			<view class="width-688 user-info u-flex u-row-between" @click="lookUser">
				<view class="u-flex">
					<image
						class="avatar"
						src="/static/images/common/headimg.jpg"
						mode=""
					></image>
					<view class="u-m-l-36">
						<view class="u-flex u-font-40">
							...
							<image
								class="user-level-icon u-m-l-36"
								:src="userInfo.level && userInfo.level.img"
								mode=""
							></image>
						</view>
						<view class="u-font-32 u-m-t-10"
							>Balance：{{ userInfo.balance }} SGD</view
						>
					</view>
				</view>
			...
			<!-- 新闻 -->
			<view v-if="false" class="width-688 news-box">
				...
				<view
					class="news-grid u-flex u-row-between"
					v-if="newsBannerList.length > 0"
				>
					<view class="img-box" @click="jumpDetail(newsBannerList[0].id)">
						<image
							class="big-img"
							:src="newsBannerList[0].img"
							mode="aspectFill"
							:style="{
								width: newsBannerList.length < 3 ? 'width-688' : '454rpx',
							}"
						></image>
						<view class="pos-desc px-p-l-30 px-p-b-20">
							<view class="px-font-16 text-line1">{{
								newsBannerList[0].title
							}}</view>
							<view class="px-font-12">{{ newsBannerList[0].author }}</view>
						</view>
					</view>
					<view
						class="small-img u-flex-col u-row-between"
						v-if="newsBannerList.length > 2"
					>
						<view class="img-box" @click="jumpDetail(newsBannerList[1].id)">
							...
							<view class="pos-desc px-p-l-18 px-p-b-10">
								<view class="px-font-12 text-line1">{{
									newsBannerList[1].title
								}}</view>
							</view>
						</view>
			...
			<view class="width-688 flot-box u-flex u-flex-wrap u-row-between">
				<view
					class="flot-item u-flex flot-box-rectangle"
					v-for="(item, index) in navList.filter(
						(item) => item.style === 'rectangle'
					)"
					:key="index"
					@click="jumpMenu(index)"
				>
					<image
						:src="'/static/images/home/home-icon' + (index + 1) + '.png'"
						mode="heightFix"
					></image>
					<view class="flot-name"
						><view>{{ item.text }}</view></view
					>
				</view>
			</view>
			<!-- 等级 -->
			<view class="width-688 px-m-t-42 flex-self-center level-box">
				...
				<view class="u-flex px-m-t-30 level-switch">
					<view v-for="(item, index) in homeData.vip_list" :key="index">
						<view
							class="vip-title"
							:class="{ 'border-main-color': currentIndex == index }"
							@click="switchLev(index)"
						>
			...
			<!-- 圆形:功能区 -->
			<view class="width-688 flot-box u-flex u-flex-wrap u-row-around">
				<view
					v-for="(item, index) in navList.filter(
						(item) => item.style === 'circle'
					)"
					class="flot-box-circle"
					:key="index"
					@click="jumpMenu(index)"
				>
					<view class="flot-item u-flex">
						<image
							:src="'/static/images/home/home-icon' + (index + 3) + '.png'"
							mode="heightFix"
						></image>
					</view>
					<view class="flot-name">{{ item.text }}</view>
				</view>
			</view>
			<!-- 合作伙伴 -->
			<view class="scroll-img u-m-t-60 partner scroll-horizontal">
				...
				<scroll-view scroll-x="true" style="height: 180rpx">
					<view class="u-flex">
						<view
							class="scroll-item u-flex"
							v-for="(item, index) in 14"
							:key="index"
						>
							<image
								:src="
									'/static/images/partner/com-' +
									index +
									'.png?' +
									Math.random()
								"
			...
			<u-popup mode="center" border-radius="12" v-model="showContentPop">
				<view class="pop-warp">
					<image
						class="poa-icon"
						src="/static/images/home/faq-icon.png"
						mode=""
						v-if="currentClick == 5 || currentClick == 6"
					></image>
					<image
						class="poa-icon"
						src="/static/images/home/faq-icon.png"
						mode=""
						v-if="currentClick == 4 || currentClick == 7"
					></image>
					<view class="pop-box">
			...
			<view class="mask-pop" v-if="showPop">
				<view class="pop-cont" @click="laterPage">
					<image
						class="img"
						:src="homeData.config && homeData.config.pop"
						mode=""
					></image>
					<image
						class="close"
						src="/static/images/home/close_green.png"
						mode=""
						@click.stop="closePop"
					></image>
				</view>
...
export default {
	...
	methods: {
		...
		laterPage() {
			let lates;
			lates = encodeURIComponent(
				JSON.stringify(this.homeData.config.event_list)
			);
		...
		jumpMenu(i) {
			...
			if (i == 3) {
				...
				cert = encodeURIComponent(
					JSON.stringify(this.homeData.config.cert_list)
				);
